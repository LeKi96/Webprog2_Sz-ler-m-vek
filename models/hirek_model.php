...
class Hirek_Model {
	public function get_data($vars): array {
		...
		try {
			...
			$sqlSelect = "select h.id, h.datum, f.felhasznalo, h.hir from hirek h
			...
			switch (true) {
				...
				case $hirTalalat >= 0:
					...
					foreach ($hirLista as $hir) {
						$kommentek = array();
						$sqlSelect = "select k.id, k.hirid, k.datum, f.felhasznalo as kommentelo, k.komment 
										from kommentek k 
